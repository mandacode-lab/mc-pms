...
func (*Project) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case project.ForeignKeys[0]: // namespace_projects
			values[i] = new(sql.NullString)
		default:
...
func (_m *Project) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case project.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullString); !ok {
				return fmt.Errorf("unexpected type %T for field namespace_projects", values[i])
			} else if value.Valid {
				_m.namespace_projects = new(string)
				*_m.namespace_projects = value.String
			}
		default:
