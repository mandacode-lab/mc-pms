...
func (_q *UserIdentityQuery) QueryService() *ServiceQuery {
...
func (_q *UserIdentityQuery) QueryUserInfo() *UserInfoQuery {
	query := (&UserInfoClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *UserIdentityQuery) Clone() *UserIdentityQuery {
	...
	return &UserIdentityQuery{
		...
		withUserInfo: _q.withUserInfo.Clone(),
...
func (_q *UserIdentityQuery) WithService(opts ...func(*ServiceQuery)) *UserIdentityQuery {
	query := (&ServiceClient{config: _q.config}).Query()
...
func (_q *UserIdentityQuery) WithUserInfo(opts ...func(*UserInfoQuery)) *UserIdentityQuery {
	query := (&UserInfoClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withUserInfo = query
	return _q
}
...
func (_q *UserIdentityQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*UserIdentity, error) {
	...
	if query := _q.withService; query != nil {
		if err := _q.loadService(ctx, query, nodes, nil,
			func(n *UserIdentity, e *Service) { n.Edges.Service = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withUserInfo; query != nil {
		if err := _q.loadUserInfo(ctx, query, nodes, nil,
			func(n *UserIdentity, e *UserInfo) { n.Edges.UserInfo = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserIdentityQuery) loadService(ctx context.Context, query *ServiceQuery, nodes []*UserIdentity, init func(*UserIdentity), assign func(*UserIdentity, *Service)) error {
...
func (_q *UserIdentityQuery) loadUserInfo(ctx context.Context, query *UserInfoQuery, nodes []*UserIdentity, init func(*UserIdentity), assign func(*UserIdentity, *UserInfo)) error {
	...
	query.Where(predicate.UserInfo(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(useridentity.UserInfoColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_identity_user_info
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_identity_user_info" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_identity_user_info" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
